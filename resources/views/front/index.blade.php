<x-app-layout>
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-md">
            <div class="p-4 bg-green-600 border-b border-gray-200">
                <div class="mb-2">
                ...
                <form action="{{ route('answer.store') }}" method="POST" role="form">
                            ...
                            <div class="border-dashed border-2 border-gray-800 p-2 my-2">
                                @if ($question->type === 'B')
                                    @if ($question->check_email)
                                        <div class="text-red-700"">{{ $email }}</div>
                                        ...
                                        <x-text-input name="answer{{ $question->id }}"
                                            ...
                                            @error("answer".($question->id))
                                                <div class="text-red-700 mt-1" role="error">{{ $url }}</div>
                                            @enderror
                                    @else
                                        <label aria-label="{{ $question->body }}" 
                                            for="answer-{{ $question->id }}">
                                        ...
                                        @error("answer".($question->id))
                                            <div class="text-red-700 mt-1" >{{ $url }}</div>
                                        @enderror
                                    @endif
                                @else
                                    <div>
                    ...
                    <div class="flex items-center justify-end mt-2">
                        <x-primary-button class="ml-3" aria-label="Soumettre" role="button">
                            {{ __('Soumettre') }}
                        </x-primary-button>
                    </div>
                </form>
